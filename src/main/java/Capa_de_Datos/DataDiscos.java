...
public class DataDiscos {
	...
	public static ArrayList<Disco> getDiscosxAutor(String autor){ 
		...
		try {
			...
			while(rs.next()){
				...
				d.setAnoLanzamiento(rs.getInt("anioLanzamiento"));
	...
	public static void addDisco (Disco disco){
		...
		try {
			...
			comando.setInt(2, disco.getAnoLanzamiento());					//anioLanzamiento
	...
	public static void updateDisco (Disco disco){
		...
		try {
			...
			comando.setInt(2, disco.getAnoLanzamiento());
	...
	//SOLO TRAE EL GENERO SI LA DESCRIPCI�N ES IGUAL! SI NO NO!
